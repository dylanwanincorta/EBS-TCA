-- these two columns are not mandatory
select vendor_id, vendor_site_id, p.*
From po_headers_all p where vendor_id is null or vendor_site_id is null;

-- the vendor_id can be derived from the vendor site
select count(*)
from po_headers_all p
inner join po_vendor_sites_all s
on p.vendor_site_id = s.vendor_site_id
where  1=1
...
select * From all_objects where object_name = 'PO_VENDORS';
...
select * FRom all_objects where object_name = 'AP_SUPPLIERS';

select * from ap_suppliers where rownum < 10;